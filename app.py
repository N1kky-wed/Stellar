...
# Removed inline _SSH_AUTH_PAGE_HTML
...
class LocalTerminalSession:
    def __init__(self, session_id, process_id, master_fd, child_pid, process_obj, user_id):
        ...
        # Start background PTY stdout reader -> publishes to Redis output channel
        self.reader_thread = threading.Thread(target=self._pty_reader_loop, daemon=True)
        self.reader_thread.start()
        
        # Start background Redis subscription listener -> listens to input & resize channels
        self.redis_thread = threading.Thread(target=self._redis_listener_loop, daemon=True)
        self.redis_thread.start()

    def _pty_reader_loop(self):
        import select
        history_key = f"stellar:terminal:history:{self.session_id}"
        output_channel = f"stellar:terminal:output:{self.session_id}"
        active_key = f"stellar:terminal:active:{self.session_id}"
            ...
            while self.active:
                ...
                if self.master_fd in r:
                    try:
                        ...
                        # Store locally and update in Redis
                ...
                if message['type'] == 'message':
                    ...
                    if channel == input_channel:
                        try:
                            ...
                            logger.error("Failed to write Redis input to master_fd: %s", e)
                        ...
                        try:
                            ...
                            logger.error("Failed to resize master_fd from Redis: %s", e)
        except Exception as e:
            logger.error("Error in Redis terminal listener loop: %s", e)
        finally:
            try:
                pubsub.unsubscribe()
            except Exception:
    ...
    def cleanup(self):
        if not self.active:
            return
        self.active = False
        logger.info("Cleaning up local PTY session_id=%s process_id=%s", self.session_id, self.process_id)
...
def api_ssh_terminal_start(process_id):
    ...
    try:
        ...
        session_obj = LocalTerminalSession(
            session_id=new_sid,
            process_id=process_id,
            master_fd=master_fd,
            child_pid=proc.pid,
...
def api_ssh_terminal_stream(session_id):
    ...
    def stream():
        ...
        try:
            ...
            logger.error("SSE stream pubsub error: %s", e)
        finally:
            try:
                pubsub.unsubscribe()
            except Exception:
...
def api_ssh_containers():
    """
    Fetch all deployed container/repository records belonging to the authenticated user.
    """
    user_id = session['user_id']
    from ssh_gateway import get_user_repos, get_container_status
    repos = get_user_repos(user_id)
    for r in repos:
        try:
            r['live_status'] = get_container_status(r['process_id'], r['app_type'])
        except Exception:
            r['live_status'] = 'stopped'
    return jsonify({'containers': repos}), 200

@app.route('/api/ssh/container/<process_id>/action', methods=['POST'])
@require_approval
def api_ssh_container_action(process_id):
    """
    Perform lifecycle or command execution actions on a user's container deployment.
    """
    user_id = session['user_id']
    data = request.get_json(silent=True) or {}
    action = data.get('action')
    app_type = data.get('app_type', 'repo')
    ...
    from ssh_gateway import get_docker_client, get_container, start_container, stop_container, restart_container

    if action == 'start':
        res = start_container(process_id, app_type, user_id)
        return jsonify({'message': res}), 200
    elif action == 'stop':
        res = stop_container(process_id, app_type, user_id)
        return jsonify({'message': res}), 200
    elif action == 'restart':
        res = restart_container(process_id, app_type, user_id)
        return jsonify({'message': res}), 200
    elif action == 'logs':
        try:
            client = get_docker_client()
        ...
        try:
            client = get_docker_client()
            container = get_container(client, process_id, app_type)
            exec_res = container.exec_run(command, user='root')
            output = exec_res.output.decode('utf-8', errors='ignore')
            return jsonify({'output': output, 'exit_code': exec_res.exit_code}), 200
        except Exception as e:
